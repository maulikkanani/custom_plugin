 ...
 * @package 	woocommerce-custom-attribute/admin/controller/wca_zippers
 * @version     1.0.0
 */
...
class wca_zippers{
    ...
    function get_single_row($id){
            global $wpdb;
            $data=array();
            $row = $wpdb->get_row("SELECT * from ".ZIPPER." where id='$id'");
            $data['name']=$row->name;
     ...
     */
     function delete($id){
         ...
         $master_id=$masters['zipper'];
         $image_dir=$category_dir.'/zipper/'.$id;
         if(delete_dir($image_dir)){
              $result=$wpdb->query("DELETE FROM ".ZIPPER." 
                                        WHERE id='$id' 
                                    ");
     ...
     */
     function upload_image(){
         ...
         $image_dir=$category_dir.'/zipper/'.$dir_name;
     ...
     */
      function delete_images(){
        ...
        $row_id=  mysql_real_escape_string($_POST[row_id]);
         ...
         $image_dir=$category_dir.'/zipper/'.$row_id;
    ...
    function active_inactive(){
         ...
         $zipper = $wpdb->get_row("SELECT * from ".ZIPPER." where id='$row_id'");
         if($zipper->status==0){
             if($uploded_image==$total_images):
                 self::do_active_inactive($row_id);
                echo 0; 
             else:   
                echo "Please upload all images for active zipper"; 
             endif; 
         }else{
             self::do_active_inactive($row_id);
             echo 1;
    ...
    function do_active_inactive($id){
        global $wpdb;
        $zipper = $wpdb->get_row("SELECT * from ".ZIPPER." where id='$id'");
        if($zipper->status==0){
