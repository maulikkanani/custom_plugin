...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
    ...
    <table class="shop_table cart wishlist_table" cellspacing="0">
        ...
        <tr>
            ...
            <th class="product-name"><span class="nobr"><?php _e( 'Product Name', 'theretailer' ) ?></span></th>
            <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?><th class="product-price"><span class="nobr"><?php _e( 'Price', 'theretailer' ) ?></span></th><?php endif ?>
            <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?><th class="stock_status"><span class="nobr"><?php _e( 'Stock Status', 'theretailer' ) ?></span></th><?php endif ?>
            <?php if( get_option( 'yith_wcwl_add_to_cart_show' ) == 'yes' ) : ?><th class="add_to_cart_wishlist"><span class="nobr"></th><?php endif ?>
        </tr>
        </thead>
		...
		foreach( $wishlist as $values ) :
			...
			if( $product_obj !== false && $product_obj->exists() ) : ?>
				<tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
					<td class="product-remove"><div><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove" title="<?php _e( 'Remove this product', 'theretailer' ) ?>">&times;</a></td>
					...
					<?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?>
						<td class="product-price">
							...
							if( $product_obj->price != '0' ) {
								...
								echo apply_filters( 'yith_free_text', __( 'Free!', 'theretailer' ) );
							}
							?>
						</td>
					<?php endif ?>
					<?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?>
						<td class="product-stock-status">
							...
							if( $stock_status == 'out-of-stock' ) {
								$stock_status = "Out";
								echo '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'theretailer' ) . '</span>';
							} else {
								$stock_status = "In";
								echo '<span class="wishlist-in-stock">' . __( 'In Stock', 'theretailer' ) . '</span>';
							}
							?>
						</td>
					<?php endif ?>
    ...
    <div class="wishlist_no_products"><?php _e( 'No products were added to the wishlist', 'theretailer' ) ?></div>
    ...
    <div class="wishlist_no_products_share"><?php yith_wcwl_get_template( 'share.php' ); ?></div>
