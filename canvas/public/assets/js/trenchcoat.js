jQuery(document).ready(function() {
    ...
    jQuery(document).bind("load-attributes", function() {
        if (jQuery('.customise_product').val() == 1) {
            ...
            jQuery.ajax({
                url: ajax_url,
                type: "POST",
                data: {action: 'get_image_box', post_id: post_id},
                success: function(html) {
                    jQuery('#main_image_layer').hide();
                    jQuery('#main_image_layer').html(html);
                    jQuery.ajax({
                        url: ajax_url,
                        type: "POST",
                        data: {action: 'get_attribute_box', post_id: post_id},
                        success: function(html) {
                            jQuery('#main_customization').html(html);
                            jQuery('#main_image_layer').show();
                            jQuery('#customize_image_layer').show();
                            jQuery('.mask').hide();
                        }
                    });
                }
    ...
    /*End:- Get attributes of category*/
    ...
    jQuery(document).bind("set-pos-1", function() {
        ...
        var front_image = $fabric_front_url + 'body_' + pos_style + '_' + pos_closuer + '_' + pos_clouser_type + '_fit' + pos_fit + '_' + pos_legth + '.png';
        var front_image_shadow = $shadow_fabric_front_url + 'body_' + pos_style + '_' + pos_closuer + '_' + pos_clouser_type + '_fit' + pos_fit + '_' + pos_legth + '.png';

        var back_image = $fabric_back_url + pos_backcut + '_cortes_fit' + pos_fit + '_' + pos_legth + '.png';
        var back_image_shadow = $shadow_fabric_back_url + pos_backcut + '_cortes_fit' + pos_fit + '_' + pos_legth + '.png';

        add_image_single_attr_front(front_image, 'front_pos_1');
        //add_image_single_attr_front(front_image_shadow, 'shadow_front_pos_1');

        add_image_single_attr_back(back_image, 'back_pos_1');
        //add_image_single_attr_back(back_image_shadow, 'shadow_back_pos_1');
    ...
    jQuery(document).bind("set-pos-2", function() {
        ...
        var img = blank_img;
        if (pockets != 0) {
            var img = 'pockets_' + pockets + '_type' + type + '_fit' + fit + '.png';
        }
        var front_image = $fabric_front_url + img;
        var front_image_shadow = $shadow_fabric_front_url + img;
        add_image_single_attr_front(front_image, 'front_pos_3');
        //add_image_single_attr_front(front_image_shadow, 'shadow_front_pos_3');
    ...
    jQuery(document).bind("set-pos-3", function() {
        ...
        var img = blank_img;
        if (type != 0) {
            var img = 'pockets_chest_type_' + type + '.png';
        }
        var front_image = $fabric_front_url + img;
        var front_image_shadow = $shadow_fabric_front_url + img;
        add_image_single_attr_front(front_image, 'front_pos_4');
        //add_image_single_attr_front(front_image_shadow, 'shadow_front_pos_4');
    ...
    jQuery(document).bind("set-pos-5", function() {
        ...
        var fit = jQuery('input[name=wca_trenchcoat_fit]:checked').val();
        var fabric_img = 'belt_' + belt + '_fit' + fit + '.png'
        var btn_img = 'boton_belt_' + belt + '_fit' + fit + '.png';
        var front_image = front_image_shadow = back_btn_image = back_btn_image_shadow = back_image = back_image_shadow = blank_image;
        if (belt != 0) {
            
            if (belt != 'sewing') {
                var front_image = $fabric_front_url + fabric_img;
                var front_image_shadow = $shadow_fabric_front_url + fabric_img;
            } else {
                var back_btn_image = $boton_url + btn_img;
                var back_btn_image_shadow = $shadow_boton_url + btn_img;
            }
            var back_image = $fabric_back_url + fabric_img;
            var back_image_shadow = $shadow_fabric_back_url + fabric_img;
        }
        add_image_single_attr_front(front_image, 'front_pos_5');
        //add_image_single_attr_front(front_image_shadow, 'shadow_front_pos_5');

        add_image_single_attr_back(back_image, 'back_pos_2');
        //add_image_single_attr_back(back_image_shadow, 'shadow_back_pos_2');
        add_image_single_attr_back(back_btn_image, 'back_pos_3');
        //add_image_single_attr_back(back_btn_image_shadow, 'shadow_back_pos_3');
    ...
    jQuery(document).bind("set-pos-6", function() {
        ...
        var fabric_img = 'shoulder.png';
        var btn_img = 'boton_shoulder_tape.png';
        var front_image = front_image_shadow = btn_image = btn_image_shadow = blank_image;
        if (shoulder == 1) {
            var front_image = $fabric_front_url + fabric_img;
            var front_image_shadow = $shadow_fabric_front_url + fabric_img;

            var btn_image = $boton_url + btn_img;
            var btn_image_shadow = $shadow_boton_url + btn_img;
        }

        add_image_single_attr_front(front_image, 'front_pos_6');
        //add_image_single_attr_front(front_image_shadow, 'shadow_front_pos_6');
        console.log(btn_image);
        add_image_single_attr_front(btn_image, 'front_pos_9');
        //add_image_single_attr_front(btn_image_shadow, 'shadow_front_pos_9');
    ...
    jQuery(document).bind("set-pos-7", function() {
        ...
        var fabric_img = 'sleeve_' + sleev + '.png';
        var btn_img = 'boton_sleeve.png';
        var front_image = front_image_shadow = btn_image = btn_image_shadow = back_image = back_image_shadow = blank_image;
        if (sleev != 0) {
            var front_image = $fabric_front_url + fabric_img;
            var front_image_shadow = $shadow_fabric_front_url + fabric_img;
            if (sleev == 'tape') {
                $front_pos['front_pos_7'].type = 'fabric';
                var back_image = $fabric_back_url + fabric_img;
                var back_image_shadow = $shadow_fabric_back_url + fabric_img;
            ...
            if (sleev == 'button') {
                $front_pos['front_pos_7'].type = 'boton';
                var btn_image = $boton_url + btn_img;
                var btn_image_shadow = $shadow_boton_url + btn_img;
        ...
        add_image_single_attr_front(front_image, 'front_pos_7');
        //add_image_single_attr_front(front_image_shadow, 'shadow_front_pos_7');

        console.log(btn_image);
        add_image_single_attr_front(btn_image, 'front_pos_8');
        //add_image_single_attr_front(btn_image_shadow, 'shadow_front_pos_8');

        add_image_single_attr_back(back_image, 'back_pos_4');
        //add_image_single_attr_back(back_image_shadow, 'shadow_back_pos_4');
    ...
    jQuery(document).bind("set-pos-8", function() {
        ...
        if (closuer == 'boton') {
            ...
            var front_image = $boton_url + but_img;
            var front_image_shadow = $shadow_boton_url + but_img;
        } else {
            var but_img = btn_style + '_zipper_' + clouser_type + '_' + coat_legth + '.png';
            var front_image = $zipper_url + but_img;
            var front_image_shadow = $shadow_zipper_url + but_img;
        }
        add_image_single_attr_front(front_image, 'front_pos_2');
        //add_image_single_attr_front(front_image_shadow, 'shadow_front_pos_2');
        jQuery(document).trigger('change-button-thread');              // for buton thread
        jQuery(document).trigger('change-button-hole-thread');         // for buton hole thread
    });
    /*End:- This is for a change sleev type*/
    ...
    jQuery(document).bind("back-pos-10", function() {
        ...
        var fabric_img = "back_lapel_1.png";
        var btn_img = "boton_belt_back_lapel_1.png";

        var back_image = back_image_shadow = btn_image = btn_image_shadow = blank_image;
        ...
        add_image_single_attr_back(back_image, 'back_pos_5');
        //add_image_single_attr_back(back_image_shadow, 'shadow_back_pos_5');
        console.log(btn_image);
        add_image_single_attr_back(btn_image, 'back_pos_6');
        //add_image_single_attr_back(btn_image_shadow, 'shadow_back_pos_6');
    ...
    jQuery(document).bind('set-fabric', function() {
        ...
        $new_fabric_color = $fabric_data[$new_fabric].color;
    ...
    jQuery(document).bind('fabric_change', function() {
        ...
        image_front_url = cat_url + '/fabric_color/' + $fabric_color + '/front';
        image_back_url = cat_url + '/fabric_color/' + $fabric_color + '/back';
...
function add_image_single_attr(img, i, canvas_obj, color) {
    fabric.Image.fromURL(img, function(img) {
        canvas_obj.item(i).setElement(img.getElement());
        img.filters.push(new fabric.Image.filters.Blend({
            color: color,
            mode: 'darken'
        }));
        img.applyFilters(canvas_obj.renderAll.bind(canvas_obj));
        canvas_obj.item(i).setCoords();
    });
}

//function add_image_single_attr_back(img, i, canvas_obj, color) {
function add_image_single_attr_back(img, pos) {
    var i = get_index_fromkey($back_pos, pos);
     console.log('Back:-'+pos+'=Pos='+i );
    var tmp_layer_data = $layer_type_data[$back_pos[pos].type];
    fabric.Image.fromURL(img, function(img) {
        $canvas_back.item(i).setElement(img.getElement());
        if(tmp_layer_data.color!=''){
            $canvas_back.item(i).filters.push(new fabric.Image.filters.Blend({
                color: tmp_layer_data.color,
                mode: 'darken'
            }));
        }
            $canvas_back.item(i).applyFilters($canvas_back.renderAll.bind($canvas_back));
        
            $canvas_back.item(i).setCoords();
    });
}

function add_image_single_attr_front(img, pos) {
    var i = get_index_fromkey($front_pos, pos);
    console.log('Fornt:-'+pos+'=Pos='+i );
    var tmp_layer_data = $layer_type_data[$front_pos[pos].type];
    fabric.Image.fromURL(img, function(img) {
        $canvas_front.item(i).setElement(img.getElement());
        if(tmp_layer_data.color!=''){
            $canvas_front.item(i).filters.push(new fabric.Image.filters.Blend({
                color: tmp_layer_data.color,
                mode: 'darken'
            }));
        }
            $canvas_front.item(i).applyFilters($canvas_front.renderAll.bind($canvas_front));
            $canvas_front.item(i).setCoords();
    });
...
function change_image_color(tmp_obj, color, canvas_obj) {
    if (color != '') {
        if (tmp_obj.filters[0]) {
            tmp_obj.filters[0]['color'] = color;
            tmp_obj.applyFilters(canvas_obj.renderAll.bind(canvas_obj));
        } else {
            tmp_obj.filters.push(new fabric.Image.filters.Blend({
                color: color,
                mode: 'darken'
            }));
            tmp_obj.applyFilters(canvas_obj.renderAll.bind(canvas_obj));
...
function set_front_layers(layers) {
    $canvas_front.clear();
    var i = 1;
    jQuery.each(layers, function(key, layer_data) {
        var color = $layer_type_data[layer_data.type].color;
        console.log('front:-'+key+'=Pos='+i );
        add_image_front(layer_data, i, color);
        i++;
    });
}
...
function add_image_front(layer_data, i, color) {
    var img = layer_data.img_src;
    if (layer_data.type == 'shadow') {
        var img = blank_image;
    }
    var tmp = fabric.Image.fromURL(img, function(img) {
        ...
        img.applyFilters($canvas_front.renderAll.bind($canvas_front));
        var oImg = img.set({left: 0, top: 0}).scale(0.9);
        $front_layes[i] = oImg;
        $canvas_front.add(oImg);
        $canvas_front.moveTo(oImg, i);
...
function set_back_layers(layers) {
    $canvas_back.clear();
    var i = 1;
    jQuery.each(layers, function(key, layer_data) {
        var color = $layer_type_data[layer_data.type].color;
        console.log('back:-'+key+'=Pos='+i );
        add_image_back(layer_data, i, color);
        i++;
    });
}
...
function add_image_back(layer_data, i, color) {
    ...
    var tmp = fabric.Image.fromURL(img, function(img) {
        ...
        img.applyFilters($canvas_back.renderAll.bind($canvas_back));
        //var oImg = img.set({left: 0, top: 0}).scale(0.9);
        var oImg = img.set({left: 0, top: 0}).scale(0.9);
        $back_layes[i] = oImg;
        $canvas_back.add(oImg);
        $canvas_back.moveTo(oImg, i);
    });
