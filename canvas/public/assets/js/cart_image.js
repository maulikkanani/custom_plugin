...
function get_index_fromkey(pos_data, key) {
    var idx = 1;
    jQuery.each(pos_data, function(data_key, data_val) {
        if (key == data_key) {
            return false;
        }
        idx++;
...
function set_static_layers(layers, canvas_obj) {
    canvas_obj.clear();
    var i = 0;
    jQuery.each(layers, function(key, layer_data) {
        var color = $layer_type_data[layer_data.type].color;
        satatic_layer_add_image(layer_data, canvas_obj, i, color);
        i++;
    });
}
...
function satatic_layer_add_image(layer_data, obj, i, color) {
    ...
    var tmp = fabric.Image.fromURL(img, function(img) {
        ...
        var oImg = img.set({left: 0, top: 0}).scale($size);
        obj.add(oImg);
        obj.moveTo(oImg, i);
    });
