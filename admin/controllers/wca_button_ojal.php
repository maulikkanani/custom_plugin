 ...
 * @package 	woocommerce-custom-attribute/admin/controller/wca_button_ojal
 * @version     1.0.0
 */
...
class wca_button_ojal{
    ...
    function get_single_row($id){
            global $wpdb;
            $data=array();
            $row = $wpdb->get_row("SELECT * from ".BUTTON_OJAL." where id='$id'");
            $data['name']=$row->name;
     ...
     */
     function delete($id){
         ...
         $master_id=$masters['button_ojal'];
         $image_dir=$category_dir.'/ojal/'.$id;
         if(delete_dir($image_dir)){
              $result=$wpdb->query("DELETE FROM ".BUTTON_OJAL." 
                                        WHERE id='$id' 
                                    ");
     ...
     */
     public static function save($id=''){
         ...
         extract($_POST);  
           ...
           if($id==''){
                $wpdb->insert(BUTTON_OJAL,$data);
                $dir_id=$wpdb->insert_id;
                $image_dir=$category_dir.'/ojal/'.$dir_id;
                if (!is_dir($image_dir)) {
                ...
                $where=array('id'=>$id);
                $wpdb->update(BUTTON_OJAL,$data,$where);
     ...
     */
     function upload_image(){
         ...
         $image_dir=$category_dir.'/ojal/'.$dir_name;
         $image_url=$category_url.'/ojal/'.$dir_name;
     ...
     */
      function delete_images(){
        ...
        $row_id=  mysql_real_escape_string($_POST[row_id]);
         ...
         $image_dir=$category_dir.'/ojal/'.$row_id;
         $image_url=$category_url.'/ojal/'.$row_id;
        ...
        if(unlink($image_path)){
                ...
                $wpdb->update(BUTTON_OJAL,$update,$where);
    ...
    function active_inactive(){
         ...
         $button_ojal = $wpdb->get_row("SELECT * from ".BUTTON_OJAL." where id='$row_id'");
         if($button_ojal->status==0){
             if($uploded_image==$total_images):
                 self::do_active_inactive($row_id);
                echo 0; 
             else:   
                echo "Please upload all images for active button ojal"; 
             endif; 
         }else{
             self::do_active_inactive($row_id);
             echo 1;
    ...
    function do_active_inactive($id){
        global $wpdb;
        $button_ojal = $wpdb->get_row("SELECT * from ".BUTTON_OJAL." where id='$id'");
        if($button_ojal->status==0){
