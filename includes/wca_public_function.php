...
function wca_get_price_from_arttributes($secected_attributes) {
    ...
    $price = 0;
    if (count($secected_attributes) > 0) {
        foreach ($secected_attributes as $key => $values) {
            $key = $key . '**NIS**' . $values;
            if (is_numeric($attribute_price[$key]) && $attribute_price[$key] > 0) {
                $price = $price + $attribute_price[$key];
            }
        }
    }
    return wca_price_digits($price);
}
?>
